...
6. Verify:
   ```bash
   set -a; . .env; set +a
   python3 .claude/scripts/gpu.py auth --providers runpod
   ```
   → `[auth] runpod: ok`
...
7. Verify:
   ```bash
   python3 .claude/scripts/gpu.py auth --providers vast
   ```
...
7. Verify:
   ```bash
   python3 .claude/scripts/gpu.py auth --providers prime
   ```
...
6. Verify:
   ```bash
   python3 .claude/scripts/gpu.py auth --providers datacrunch
   ```
...
python3 .claude/scripts/gpu.py auth

# Cross-provider price compare
python3 .claude/scripts/gpu.py price 'NVIDIA H100 80GB HBM3'   # RunPod's id
python3 .claude/scripts/gpu.py price 'H100_SXM'                # Vast's id

# Spend dashboard (humans)
python3 .claude/scripts/gpu.py dashboard
...
```bash
python3 .claude/scripts/gpu.py create --gpu-type 'RTX 3090' --hours 0.1 --yes
# → router picks the cheapest available spot across configured providers,
#   prints SSH endpoint when running

# When done:
python3 .claude/scripts/gpu.py terminate --provider <name> <pod_id> --yes
```
...
- Run manually: `python3 .claude/scripts/gpu.py reconcile -v`
