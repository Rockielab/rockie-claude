#!/usr/bin/env bash
# SessionStart hook — runs once at the beginning of every Claude Code session.
#
# Three jobs:
...
#      <repo>/.claude/memory/rules-compiled.md so CLAUDE.md can reference
#      them and the rules load into context.
#
# Exits silently if the repo has no .claude/ dir or we're not in a git repo.
...
SCRIPTS="$HOME/.claude/scripts/memory"
