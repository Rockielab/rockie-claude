...
#     compact source, since the hook carries no matcher), falls back
#     cleanly on missing/malformed SKILL.md, skips skill dirs with none,
#     silently no-ops with no skills/ dir, caps large counts at 40
#
...
# ── Regression coverage: pre-train-gate .py-extraction bug (dogfooding,
# reported on a downstream install) ──────────────────────────────────────
...
# .claude/scripts/calibration.py --help` is the real launch, but a later
# `find -name "journal.py"` clause used to hijack extraction and block it.
REPRO_CMD='ls .claude/scripts/ | head -20; echo "---"; ls STATE.md EXPERIMENT_LOG.md 2>&1; echo "---"; python3 .claude/scripts/calibration.py --help 2>&1 | head -10 || find .claude -name "calibration.py" -o -name "journal.py" | head -5'
...
bash "$PROJ/.claude/scripts/dry_run_gate.sh" register "$PROJ/quoted_train.py" >/dev/null
echo "$IN" | bash "$PROJ/.claude/hooks/pre-train-gate.sh" >/dev/null 2>&1
assert "pre-train-gate: quoted script path passes once registered (quotes stripped correctly)" "0" "$?"

# (e) Harness-internal utility scripts (.claude/scripts/*.py in the
# installed layout) must never trip the gate.
IN=$(python3 -c "import json;print(json.dumps({'tool_input':{'command':'python3 .claude/scripts/calibration.py --help','cwd':'$PROJ'}}))")
echo "$IN" | bash "$PROJ/.claude/hooks/pre-train-gate.sh" >/dev/null 2>&1
assert "pre-train-gate: .claude/scripts/calibration.py passes (harness-script exemption)" "0" "$?"

IN=$(python3 -c "import json;print(json.dumps({'tool_input':{'command':'python3 .claude/scripts/journal.py --help','cwd':'$PROJ'}}))")
echo "$IN" | bash "$PROJ/.claude/hooks/pre-train-gate.sh" >/dev/null 2>&1
assert "pre-train-gate: .claude/scripts/journal.py passes (harness-script exemption)" "0" "$?"
...
bash "$PROJ/.claude/scripts/dry_run_gate.sh" register "$PROJ/src3/train.py" >/dev/null
echo "$IN" | bash "$PROJ/.claude/hooks/pre-train-gate.sh" >/dev/null 2>&1
...
ERR=$(echo "$IN" | bash "$PROJ/.claude/hooks/pre-train-gate.sh" 2>&1 >/dev/null)
echo "$IN" | bash "$PROJ/.claude/hooks/pre-train-gate.sh" >/dev/null 2>&1
...
[ -x "$PROJ/.claude/scripts/clean-finalize.sh" ] && ok "clean-finalize: script installed and executable" || fail "clean-finalize: missing or not executable"
rm -f "$PROJ"/.claude/.state/clean-ok-*
NUDGE_OUT=$(bash "$PROJ/.claude/scripts/clean-finalize.sh" testhash123 2>&1 >/dev/null)
[ -f "$PROJ/.claude/.state/clean-ok-testhash123" ] && ok "clean-finalize: writes sentinel at expected path" || fail "clean-finalize: sentinel missing"
...
echo "$NUDGE_OUT" | grep -q 'saml212/rockie-claude' && ok "clean-finalize: nudge references saml212/rockie-claude" || fail "clean-finalize: nudge missing upstream repo"
...
# config, and the real `git commit` two lines down (unlike the rest of this
# suite, which only feeds JSON to hooks) needs one or fails with "empty
# ident name", which cascades into `git worktree add` never creating
# $WTBASE/wt and four unrelated assertions failing below.
...
# find-skills ships and installs like any other skill.
python3 - "$PROJ/.claude/skills/find-skills/SKILL.md" <<'PY' && ok "find-skills SKILL.md installs with valid frontmatter" || fail "find-skills frontmatter"
...
mkdir -p "$ONRAMP_PROJ/.claude"
rsync -a --exclude='__pycache__' "$ROCKIE/project-harness/" "$ONRAMP_PROJ/.claude/" >/dev/null
SR="$ONRAMP_PROJ/.claude/scripts/session_report.py"
...
if [ -f "$ONRAMP_PROJ/.claude/.state/catalog-onramp-shown" ]; then
...
mkdir -p "$LOGGEDOUT_PROJ/.claude"
rsync -a --exclude='__pycache__' "$ROCKIE/project-harness/" "$LOGGEDOUT_PROJ/.claude/" >/dev/null
LO_OUT=$(env PATH="$FAKEBIN:$NOROCKIE_PATH" FAKE_ROCKIE_EXIT=2 \
  python3 "$LOGGEDOUT_PROJ/.claude/scripts/session_report.py" 2>/dev/null); LO_RC=$?
if [ "$LO_RC" = "0" ]; then
...
FS="$ROCKIE/project-harness/skills/find-skills/SKILL.md"
...
CG="$ROCKIE/project-harness/skills/cli-guidance.md"
...
# Both CLAUDE.md templates carry the on-ramp.
if grep -q 'find-skills' "$ROCKIE/claude-md/CLAUDE.md.template" \
   && grep -q 'find-skills' "$ROCKIE/claude-md/ml-research.md"; then
  ok "both CLAUDE.md templates point at the skill catalog"
else
...
# ── 17. Installed skills inventory (post-compaction rediscovery) ─────────
# SessionStart in this harness carries no `matcher`, so it fires on every
# source Claude Code emits — startup, resume, clear, AND compact (see
...
mkdir -p "$SKILLS_PROJ/.claude/scripts"
cp "$ROCKIE/project-harness/scripts/session_report.py" "$SKILLS_PROJ/.claude/scripts/session_report.py"

mkdir -p "$SKILLS_PROJ/.claude/skills/well-formed"
cat > "$SKILLS_PROJ/.claude/skills/well-formed/SKILL.md" <<'EOF'
---
name: well-formed
...
# "description".
mkdir -p "$SKILLS_PROJ/.claude/skills/malformed-frontmatter"
cat > "$SKILLS_PROJ/.claude/skills/malformed-frontmatter/SKILL.md" <<'EOF'
---
...
mkdir -p "$SKILLS_PROJ/.claude/skills/no-frontmatter"
cat > "$SKILLS_PROJ/.claude/skills/no-frontmatter/SKILL.md" <<'EOF'
# no-frontmatter
...
mkdir -p "$SKILLS_PROJ/.claude/skills/no-skill-md/scripts"
echo "print('hi')" > "$SKILLS_PROJ/.claude/skills/no-skill-md/scripts/helper.py"

SKILLS_OUT=$(python3 "$SKILLS_PROJ/.claude/scripts/session_report.py" 2>&1)
...
# Missing skills/ dir entirely — silent skip, no crash, no empty header.
NOSKILLS_PROJ=$(mktemp -d -t smoke-noskills-XXXXXX)
mkdir -p "$NOSKILLS_PROJ/.claude/scripts"
cp "$ROCKIE/project-harness/scripts/session_report.py" "$NOSKILLS_PROJ/.claude/scripts/session_report.py"
NOSKILLS_OUT=$(python3 "$NOSKILLS_PROJ/.claude/scripts/session_report.py" 2>&1); NOSKILLS_RC=$?
assert "missing skills/ dir: session report still exits 0" "0" "$NOSKILLS_RC"
assert "missing skills/ dir: no Installed skills section emitted" \
...
mkdir -p "$CAP_PROJ/.claude/scripts"
cp "$ROCKIE/project-harness/scripts/session_report.py" "$CAP_PROJ/.claude/scripts/session_report.py"
for i in $(seq -w 1 45); do
  d="$CAP_PROJ/.claude/skills/fixture-skill-$i"
  mkdir -p "$d"
  cat > "$d/SKILL.md" <<EOF
---
...
CAP_OUT=$(python3 "$CAP_PROJ/.claude/scripts/session_report.py" 2>&1)
